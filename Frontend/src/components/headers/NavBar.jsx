...
const theme = createTheme({
  palette: {
    primary: {
      main: "#91ff33", // Set the primary color
    },
    secondary: {
      main: "#78b721", // Set the secondary color
    },
  },
});
    ...
    >
      <div className="lg:w-[95%] mx-auto sm:px-6 lg:px-6">
        <div className="flex px-4 items-center justify-between py-4">
          ...
          >
            <div className={``}>
              <h1 className="text-2xl font-Cinzel  inline-flex gap-3  items-center font-bold dark:text-white">
              Yoga Connect <img src="/yoga-logo.jpg" alt="" className='w-8 h-8 object-contain rounded-full' /></h1>
              <p className='font-semi-bold text-[13px] tracking-[3px]  mt-1 dark:text-white'>Connect with yourself</p>
            </div>
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden mt-2 w-full bg-black">
            {navLinks.map((link) => (
              <li key={link.route}>
                <NavLink
                  className={({ isActive }) =>
            ...
            {user && (
              <li>
                <NavLink
                  className="font-normal px-1 py-1 bg-secondary text-white  rounded-lg"
                  onClick={handelLogout}
                >
                  Logout
                </NavLink>
